...
class UserScript(UserScriptV1):
    ...
    The following arguments should be specified in the user script arguments::
        ...
        (str) resource_type: cpu or memory
        (list) resource_util_list: the target resource utilization of HPA
    ...
    def update_target_mem_util_in_hpa(self, mem_util):
        '''
        Update target cpu utilization in hpa.
        One possible exeption happen, see: https://github.com/kubernetes-client/python/issues/1098
        '''
        if self.user_args["autoscaling_version"] == "v1":
            body = {
                "spec": {
                    "targetMemoryUtilizationPercentage": mem_util
                }
            }
        elif self.user_args["autoscaling_version"] == "v2beta2":
            body = {
        ...
        try:
            ...
            self._log.error("update hpa error: %s" % str(e))
            raise TestRunError("update target cpu util %d fail!" % mem_util)

        self._log.info("update target mem util %d complete!" % mem_util)
    ...
    def force_scale_to_min_replicas(self):
        body = {
            'spec': {'replicas': self.min_replicas}
        }

        deployment = self.apps_v1_api.patch_namespaced_deployment_scale(
    ...
    def scale_down_deployment(self):
        ...
        # force scale in to min replicas due to memory consumption can't restore back to original value even stop http request
        if self.user_args["resource_type"] == "memory":
            self.force_scale_to_min_replicas()
            # sleep some time to make sure scale in complete
            time.sleep(2)
    ...
    def get_pod_mem_request(self):
        mem_request = 0
        pod_list = self.core_v1_api.list_namespaced_pod(
            namespace=self.user_args["k8s_namespace"])
        for item in pod_list.items:
            if item.metadata.name.startswith(self.deployment_name):
                mem_str = item.spec.containers[0].resources.requests["memory"]
                mem_request = int(mem_str.rstrip('Mi'))

        return mem_request

    def get_pod_rc_request(self):
        cpu_str, mem_str = (None, None)
        pod_list = self.core_v1_api.list_namespaced_pod(
            namespace=self.user_args["k8s_namespace"])
        for item in pod_list.items:
            if item.metadata.name.startswith(self.deployment_name):
                cpu_str = item.spec.containers[0].resources.requests.get("cpu")
                mem_str = item.spec.containers[0].resources.requests.get("memory")
    ...
    def start_http_client_rate(self, rate):
        ...
        if self.user_args["resource_type"] == "cpu":
            payload = {
            ...
            payload = {
                "target_url": "http://" + self.http_server_svc_ip + "/memload.php",
                "count": 100,
                "rate": rate,
            }
    ...
    def stop_http_client(self):
        ...
        # wait for client clean up the works in threadpool
        time.sleep(3)
        return
    ...
    def run(self, user_args: dict) -> None:
        ...
        # query pod cpu and memory request
        cpu_request, mem_request = self.get_pod_rc_request()
        ...
        self._iq.write("Chart configuration", "pod_cpu_request: {}, pod_mem_request: {}, deployment_init_replicas: {}, hpa_min_replicas: {}, hpa_max_replicas: {}".format(
            cpu_request, mem_request, self.deployment_init_replicas, self.min_replicas, self.max_replicas))

        self._iq.write(label="Start HPA test", value="")

        try: 
            for rc_util in self.user_args["resource_util_list"]:
                self._iq.write(label="Start {} util".format(self.user_args["resource_type"]),
                            value="resource_util: {}".format(rc_util))
                if self.user_args["resource_type"] == "cpu":
                    self.update_target_cpu_util_in_hpa(rc_util)
                else:
                    self.update_target_mem_util_in_hpa(rc_util)
                # sleep some time to make hpa effective
                time.sleep(10)
                self.trigger_and_monitor_autoscale_status()
                self._iq.write(label="End {} util".format(self.user_args["resource_type"]),
                            value="resource_util: {}".format(rc_util))

                # stop cpu load to make deployment scale down to min replicas
